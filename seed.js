// seed.js — populates the database with themes and card data
// This runs automatically on first startup if the database is empty.
// To add more card sets, add them to the THEMES object below and redeploy.

module.exports = function seed(db) {
  console.log('Seeding database with themes and cards...');

  const insertTheme = db.prepare('INSERT OR IGNORE INTO themes (name, slug) VALUES (?, ?)');
  const insertCard = db.prepare(`
    INSERT INTO cards (theme_slug, set_letter, card_number, songs)
    VALUES (?, ?, ?, ?)
  `);
  ...
  // ── Helper: build a card grid (24 songs + free space at index 12) ────────
  function buildCard(pool, seed) {
    const shuffled = [...pool].sort(() => {
      seed = (seed * 1664525 + 1013904223) & 0xffffffff;
      return (seed >>> 0) / 0xffffffff - 0.5;
    });
    const picked = shuffled.slice(0, 24);
    const grid = [...picked.slice(0, 12), null, ...picked.slice(12)];
    return grid;
  }

  // ── Helper: generate a set of cards with a given number sequence ─────────
  function generateSet(themeSlug, setLetter, pool, numberSequence) {
    const seedBase = themeSlug.charCodeAt(0) * 1000 + setLetter.charCodeAt(0) * 100;
    numberSequence.forEach((cardNum, pageIdx) => {
      const grid = buildCard(pool, seedBase + pageIdx);
      insertCard.run(themeSlug, setLetter, cardNum, JSON.stringify(grid));
    });
  }

  // ── Helper: shuffle array with a given seed ──────────────────────────────
  ...
  // ── 80s theme ────────────────────────────────────────────────────────────
  insertTheme.run('Hit the 80s', '80s');
  generateSet('80s', 'A', songs80s, sequential);
  generateSet('80s', 'B', songs80s, seededShuffle(sequential, 8001));
  generateSet('80s', 'C', songs80s, seededShuffle(sequential, 8002));
  generateSet('80s', 'D', songs80s, seededShuffle(sequential, 8003));

  // ── Old school rock theme ────────────────────────────────────────────────
  insertTheme.run('Old School Rock', 'rock');
  generateSet('rock', 'A', songsRock, sequential);
  generateSet('rock', 'B', songsRock, seededShuffle(sequential, 9001));
  generateSet('rock', 'C', songsRock, seededShuffle(sequential, 9002));
  generateSet('rock', 'D', songsRock, seededShuffle(sequential, 9003));

  console.log('Database seeded. Themes: 80s, rock. Cards: 140 x 4 sets each.');
};
